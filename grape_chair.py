...
for state in permutations(tiles_master): 
  A = state[0]
  B = state[1]
  C = state[2]
  D = state[3]
  ...
  F = state[5]
  G = state[6]
  if ( first(A,B) == second(C,D,E) == third(F,G) == fourth(C, F)
  == fifth(A,D) == sixth(B,E,G) == seventh(D,G) == eighth(F,D,B) ):
    print ("A: %d; B: %d; C: %d; D: %d; E: %d; F: %d; G: %d" % 
    (A, B, C, D, E, F, G) )
